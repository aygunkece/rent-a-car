    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                <?php echo __('translate-manager.title')?>
            </a>
        </div>
    </div>
</header>
<div class="container-fluid">
    ...
    <p>
                ...
                <div class="row">
                    <div class="col-sm-3">
                        <select name="replace" class="form-control">
                            <option value="0"><?php echo __('translate-manager.import_new')?></option>
                            <option value="1"><?php echo __('translate-manager.import_replace')?></option>
                        </select>
                    </div>
                    <div class="col-sm-2">
                    <button type="submit" class="btn btn-success btn-block"  data-disable-with="Loading.."><?php echo __('translate-manager.import_group')?></button>
                    </div>
                </div>
            </div>
        </form>
        ...
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." ><?php echo __('translate-manager.publish_translate_btn')?></button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default"><?php echo __('translate-manager.back_btn')?></a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p><?php echo __('translate-manager.choose_group_desc')?></p>
        ...
        <div class="form-group">
            <label><?php echo __('translate-manager.new_group_name_input')?></label>
            <input type="text" class="form-control" name="new-group" />
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="<?php echo __('translate-manager.new_group_name_btn')?>" />
        </div>
    </form>
    <?php if($group): ?>
        <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <label><?php echo __('translate-manager.new_key_input')?></label>
                <textarea class="form-control" rows="3" name="keys" placeholder="Add 1 key per line, without the group prefix"></textarea>
            </div>
            <div class="form-group">
                <input type="submit" value="<?php echo __('translate-manager.new_key_btn')?>" class="btn btn-primary">
            </div>
        </form>
        <?php if (1>2) { ?>
                <div class="row">
    ...
    <?php else: ?>
        <fieldset>
            <legend><?php echo __('translate-manager.locales_title')?></legend>
            <p>
                <?php echo __('translate-manager.locale_sub_title')?>
            </p>
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                ...
                <div class="form-group">
                    <p>
                        <?php echo __('translate-manager.new_locale_input')?>
                    </p>
                    <div class="row">
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding.."><?php echo __('translate-manager.new_locale_btn')?></button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <fieldset>
            <legend><?php echo __('translate-manager.export_all_title')?></legend>
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Publishing.." ><?php echo __('translate-manager.publish_all_btn')?></button>
            </form>
        </fieldset>
